...
flags = tf.app.flags
FLAGS = flags.FLAGS
flags.DEFINE_float('', 0.001, 'Initial learning rate.')
...
flags.DEFINE_float('edge_dropout', 0.5, 'Dropout for individual edges in training graph')
flags.DEFINE_float('autoregressive_scalar', 0.5, 'Scalar for Graphite')
...
for test in range(FLAGS.test_count):
    ...
    with tf.name_scope('optimizer'):
        opt = OptimizerVAE(preds=model.reconstructions,
                           labels=tf.reshape(
                               tf.sparse_tensor_to_dense(placeholders['adj_orig'], validate_indices=False), [-1]),
    ...
    logging.basicConfig(
        level=logging.DEBUG,
        filename='{}_{}_{}.log'.format(FLAGS.dataset, FLAGS.model, FLAGS.features)
    )
    logger = logging.getLogger(FLAGS.model)
    ...
    for epoch in range(FLAGS.epochs):
        ...
        if FLAGS.verbose:
            logger.info(
                'Accuracy: {:5.5f} +- {:5.5f}'.format(roc_score, ap_score))
